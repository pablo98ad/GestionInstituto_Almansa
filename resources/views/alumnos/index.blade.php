  ...
  <div class="row mt-2 ">
    ...
    <div class="card col-md-3 col-sm-6 col-12 mt-1 ">
      <div class="card-body m-0 p-0 mt-2 mb-2">
        <h2 class="card-title d-inline">{{$alumno->nombre}}</h2>
        <img draggable="false" class="card-img-top w-25 d-inline border mb-1" src="{{url('/').'/storage/'.$alumno->rutaImagen/*url('../').'/storage/app/public/'.$profesor->rutaImagen*/}}" alt="">
        <h5 class="card-subtitle mb-2 text-muted">{{$alumno->apellidos}}</h5>
        ...
        <a class='btn btn-success' href='alumno/{{$alumno->id}}' role='button'>Ver</a>
        <!--<a class='btn btn-warning' href='horario/alumno/{{$alumno->id}}' role='button'>Horario</a>-->
        @if (Auth::check())
        <a class='btn btn-primary' href='alumno/{{$alumno->id}}/edit' role='button'>Editar</a>
        <div class="d-inline">
